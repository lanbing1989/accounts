...
$report_title = "现金流量表";
$report_date = date('Y-m-t', strtotime("$sel_year-$sel_month-01"));
$report_unit = "元";
$report_template = "会小企03表";
$month1 = date('Y-m-01', strtotime("$sel_year-$sel_month-01"));
$month2 = date('Y-m-t', strtotime("$sel_year-$sel_month-01"));
...
// 单项金额（年累计/本月），并缓存
function calc_cashflow_item($item, $col, $date1, $date2, $month1, $month2, $book_id, &$cache) {
...
if (isset($_GET['export']) && $_GET['export'] == 'csv') {
    ...
    header('Content-Disposition: attachment;filename=现金流量表_'.$sel_year.'_'.$sel_month.'.csv');
    ...
    foreach($cashflow_items as $item) {
        ...
        $year_val = calc_cashflow_item($item, 'year', $date1, $date2, $month1, $month2, $currentBookId, $cache);
        $month_val = calc_cashflow_item($item, 'month', $date1, $date2, $month1, $month2, $currentBookId, $cache);
        $row = [
            $item['label']??'', $item['idx']??'',
    ...
    <div id="print-area">
        ...
        <table border="1" cellspacing="0" cellpadding="4" style="border-collapse:collapse;font-size:15px;width:100%;">
            ...
            foreach($cashflow_items as $item) {
                ...
                $cache[$item['idx']]['month'] = $month_val;
